# Hamiltonian-derived displacement constructors.
#
# These methods extend the closed-form displacement API with exact equilibrium and
# finite-harmonic responses inferred from a one-mode reference Hamiltonian.

function fock_displacement_from_cnum(d::Op, c::CNum)
...
function validate_fock_time(data::FockLinearReference, t::Num)
...
function Displace(a::Op, Hlin::QAdd)
    ...
    return fock_displacement_from_cnum(d, static_fock_displacement(data))
end
...
function Displace(a::Op, Hlin::QAdd, t::Num)
    ...
    return timed_transform(fock_displacement_from_cnum(d, amplitude), gauge, tt)
end

function quadrature_displacement_from_cnum(x::Op, p::Op, cx::CNum, cp::CNum)
    return static_transform(
        pair_rules(
...
function validate_quadrature_time(data::QuadratureLinearReference, t::Num)
    variable = SymbolicUtils.unwrap(t)
    for (coefficient, name) in (
            (data.quadratic_x, "x²"),
...
function Displace(x::Op, p::Op, Hlin::QAdd)
    ...
    return quadrature_displacement_from_cnum(x, p, displacement.x, displacement.p)
end
...
function Displace(x::Op, p::Op, Hlin::QAdd, t::Num)
    ...
    tt = time_or_throw(t)
    data = linear_quadrature_hamiltonian(Hlin, x, p)
    validate_quadrature_time(data, tt)
    ...
    gauge = quadrature_displacement_gauge(
        ...
        neg_cnum(transformed_x_coefficient), neg_cnum(transformed_p_coefficient),
    )
    static = quadrature_displacement_from_cnum(x, p, displacement.x, displacement.p)
